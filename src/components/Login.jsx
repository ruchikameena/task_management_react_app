...
const Login = () => {
  ...
  //to handle the login part of application
  const handleLogin = async (e) => {
    e.preventDefault();
    try {
      const userCred = await signInWithEmailAndPassword(auth, email, password);
      const user = userCred.user;
      //prevent user if there email is not verified
      if (!user.emailVerified) {
        alert('Your email is not verified, Please verify and try again later.');
        return;
      }
      //else link to the dashboard
      navigate('/dashboard');
    } catch (err) {
  ...
  //help users to change their password
      ...
      <div className='auth_main_right'>
        <div className="auth_right_content">
          ...
          <form onSubmit={handleLogin} >
            ...
            <input value={password} onChange={(e) => setPassword(e.target.value)} type="password" placeholder="Password" required className='auth_main_input'/>
            <button type="submit" className='auth_main_btn'>Login</button>
          </form>
          <button onClick={handleForgotPassword} className='auth_main_btn2'>Forgot Password?</button>
          <p>Not registered?{' '}
            <span onClick={() => navigate('/register')} className='auth_main_navigate'>Register here</span>
          </p>
        </div>
      </div>
