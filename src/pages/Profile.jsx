...
const Profile = () => {
  ...
  //taking 0 = Sunday and 6 = Saturday
  ...
  return (
    <div className='profile_main'>
      ...
      <button onClick={handleLogout} className='profile_main_btn'>Logout</button>
          ...
          {monthDays.map((dateObj) => {
            ...
            return (
              <div key={dateStr} title={dateStr}
                style={{height: '40px',width: '100%',backgroundColor: loggedIn ? '#4caf50' : '#d1d0d0',border: isToday ? '3px double #000' : '1px solid #ccc',color: 'white',fontWeight: isToday ? 'bold' : 'normal',textAlign: 'center',borderRadius: '6px',padding: '10px',fontSize: '14px',}}>{dateObj.getDate()}
              </div>
            );
          })}

        </div>
        <p style={{ fontSize: '13px', marginTop: '0.5rem', textAlign: 'center' }}>🟩 Logged In &nbsp;|&nbsp; ⬜️ Missed &nbsp;|&nbsp; 🔲 Today</p>
